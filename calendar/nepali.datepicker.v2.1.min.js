...
function showNdpCalendarBox(a)
{
    if(npdCalendarVisible)hideCalendarBox(!1);
        ...
        var b=jQuery("#"+a).val();jQuery("#ndp-target-id").html(a);var c=jQuery("#"+a).offset();
        jQuery("#ndp-nepali-box").css("top",c.top+jQuery("#"+a).outerHeight()),
        jQuery("#ndp-nepali-box").css("left",c.left),
        showCalendar(b),npdCalendarVisible=!0}}
        ...
        {
            showCalendar((ndpAttr.npdYear?jQuery("#npd-year-select").val():a)+"-"+jQuery("#npd-month-select").val()+"-01")
        }
        function performSelectYear(a)
        {
            showCalendar(jQuery("#npd-year-select").val()+"-"+(ndpAttr.npdMonth?jQuery("#npd-month-select").val():a)+"-01")
        }
        function getMonthSelect(a,b)
        {
            var c=getNepaliMonthsInNepali(),d='<select id="npd-month-select" onchange="performSelectMonth('+b+')">';
            return c.forEach(function(b,c){d+='<option value="'+(c+1)+'"'+(c+1==a?" selected":"")+">"+b+"<li>"}),d+="<select>"
        }
        function getYearSelect(a,b,c)
        {
            console.log(c);var d='<select id="npd-year-select" onchange="performSelectYear('+a+')">';
            for(i=Math.round(c/2)>0&&parseInt(b)-Math.round(c/2)>=2e3?parseInt(b)-Math.round(c/2):2e3;i<=(Math.round(c/2)>0&&parseInt(b)+Math.round(c/2)<=2090?parseInt(b)+Math.round(c/2):2090);i++)d+='<option value="'+i+'"'+(b==i?" selected":"")+">"+englishNo2Nep(i)+"<li>";return d+="<select>"
        }
