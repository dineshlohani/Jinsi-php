...
class Aanyaprastabana
{
	protected static $table_name = "aanya_prastabana";
	protected static $db_fields = array('id','anya_prastabana_id','anya_prastabana_entry_date','prastabana_prakar_id','miti','miti_english');
	public $id;
	public $anya_prastabana_id;
        public $anya_prastabana_entry_date;
        public $prastabana_prakar_id;
        public $miti;
        public $miti_english;
        ...
        public function resetAutoIncrement()
	{
        ...
        public static function find_by_aanya_prastabana_id($kharid_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where anya_prastabana_id={$kharid_id} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
          public static function get_max_anya_prastabana_id()
        {
            global $database;
            $sql = "select max(anya_prastabana_id) from ".self::$table_name. " ";
	...
	{
		$result_array=self::find_by_sql("select distinct(organization_id) from ".self::$table_name. " where kharid_id={$id} ");
		return $result_array;
	}
        public static function find_by_anya_prastabana_id($kharid_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where anya_prastabana_id={$kharid_id}");
		return $result_array;
	}
        public static function find_by_aanya_prastabana_id_org_id($kharid_id=0,$org_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where anya_prastabana_id={$kharid_id} and organization_id={$org_id} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
        public static function find_all_org_id($kharid_id=0)
        {
            $result_array=self::find_by_sql("select * from ".self::$table_name. " where anya_prastabana_id={$kharid_id}");
            $org_ids = array();
            foreach($result_array as $result)
            {
                array_push($org_ids, $result->organization_id);
            }
            return $org_ids;
        }
