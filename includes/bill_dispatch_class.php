...
require_once('database.php');
    ...
    class BillDispatch
{
	protected static $table_name = "bill_dispatch";
	protected static $db_fields = array('id','english_date','rashid_to','department_id',
                'description_id','user_id','khata_id','rashid_range');
	public $id;
        public $english_date;
        public $rashid_to;
...
// Common database method
	public static function find_all()
	{
		...
		return self::find_by_sql("select * from ".self::$table_name." order by english_date desc");
          ...
          public static function get_new_khata_id($department_id)
       {
        global $database;   
        ...
        $sql="select khata_id  from ".self::$table_name." where department_id = {$department_id} ";
        ...
        if(empty($row['khata_id']))
          ...
          public static function get_max_from($date)
	{
		global $database;
	...
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where english_date='".$date."' limit 1");
	...
	{
		global $database;
		$sql = "select sum(total) from ".self::$table_name." where description_id=".$description_id;
		$result_set = $database->query($sql);
		$row = $database->fetch_array($result_set);
		return array_shift($row);
	}
