...
class DakhilaItemDetails
{
	protected static $table_name = "dakhila_item_details";
	protected static $db_fields = array('id','item_id','category','rate','rate_vat',
            'qty','vat_status','total','vat_amount','vat_total','extra_amount','gross_total','dakhila_id');
		public $id;
        public $item_id;
        public $category;
        public $rate;
	...
	public static function find_all()
	{
        ...
        public function find_by_item_id_rate_category_of_max_dakhila($item_id,$category,$rate)
        {
            global $database;
            $sql="select MAX(dakhila_id) from ".self::$table_name. " where rate=$rate or rate_vat=$rate and item_id=$item_id and category=$category   limit 1";
            $result_set = $database->query($sql);
            $row = $database->fetch_array($result_set);
            return  $dakhila_id  =  array_shift($row);
        }
	...
	public static function find_by_id($id=0)
	{
        ...
        public static function find_by_dakhila_id($dakhila_id=0)
	{
		global $database;
		$result_array = self::find_by_sql("select * from ".self::$table_name. " where dakhila_id={$dakhila_id} ");
		return $result_array;
	}
