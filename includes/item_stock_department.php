...
class ItemStockDepartment
{
	protected static $table_name = "item_stock_department";
	protected static $db_fields = array('id','department_id','stock','category',
            'item_id','rate','stock_date','stock_date_english','hastantaran_id');
	public $id;
	...
	public $item_id;
        ...
        public static function get_new_hastantaran_id()
        {
            global $database;
            $sql= "select max(hastantaran_id) from ".self::$table_name;
            $result_set = $database->query($sql);
            $row = $database->fetch_array($result_set);
            if(empty($result_set))
            {
        ...
        public static function find_by_rate_category_item_id($id=0)
	{
	    ...
	    return $result_array;
          ...
          public static function find_by_has_id($adesh_id=0)
	{
		global $database;
		$result_array = self::find_by_sql("select * from ".self::$table_name. " where hastantaran_id={$adesh_id} ");
		return $result_array;
	}
