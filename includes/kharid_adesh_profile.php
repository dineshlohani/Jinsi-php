...
class KharidAdeshProfile
{
	protected static $table_name = "kharid_adesh_profile";
	protected static $db_fields = array('id','maag_id','date_english','date_nepali','enlist_id','entry_date_nepali','entry_date_english', 'non_enlist_name', 'non_enlist_address','is_suchikrit', 'nirnaya_no', 'nirnaya_nepali_date', 'nirnaya_english_date','total_amount','total_vat_amount','total_with_vat');
	public $id;
    public $maag_id;
	public $date_english;
    public $date_nepali;
    public $enlist_id;
    public $entry_date_nepali;
	public $entry_date_english;
    public $is_suchikrit;
    public $non_enlist_name;
    public $non_enlist_address;
    public $nirnaya_no;
    public $nirnaya_nepali_date;
    public $nirnaya_english_date;
    public $total_amount;
    public $total_vat_amount;
    public $total_with_vat;
        // public $discount_amount;
        // Common database method
        public static function find_by_maag_id($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where maag_id={$id} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
         public static function check_maag_id($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where maag_id={$id} limit 1");
		return !empty($result_array)? TRUE : FALSE;
	}
          public static function get_enlist_from_maag($maag_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where maag_id={$maag_id} limit 1");
		if(!empty($result_array))
		{
		    $result= array_shift($result_array);
		    return $result->enlist_id;
		}
		else
		{
		   return  FALSE;    
