...
class MachinarydetailsProfile
{
	protected static $table_name = "machinary_profile";
	protected static $db_fields = array('id','machine_id','type','model','darta_no','miti','miti_english'
            ,'total_distance','quantity','total_amount');
	public $id;
	public $machine_id;
        public $type;
        public $model;
        ...
        // Common database method
           public static function get_max_darta_no()
        {
            global $database;
            $sql = "select max(darta_no) from ".self::$table_name. " ";
	...
	public static function find_all()
	{
		...
		return self::find_by_sql("select * from ".self::$table_name);
         ...
         public static function find_by_machinary_id_date($id=0,$miti)
	{
		global $database;
                $sql="select * from ".self::$table_name. " where machine_id={$id}  and miti_english='".$miti."'";
               $result_array=self::find_by_sql($sql);
		return $result_array;
	}
          public static function find_by_machinary_id($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where machine_id={$id} ");
		return $result_array;
	}
         public static function find_by_machine_id($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where machine_id={$id} limit 1");
