...
class MemberInvited 
{
	protected static $table_name = "member_invited";
	protected static $db_fields = array('id','member','pad','profile_id','type');
	public $id;
	public $member;
        public $pad;
        public $profile_id;
	...
	public static function find_by_id($id=0)
	{
        ...
        public static function find_by_profile_id_sanyojak_id($profile_id=0,$sanyojak_id=0)
        {
            $result_array=self::find_by_sql("select * from ".self::$table_name. " where profile_id={$profile_id} and worker_id={$sanyojak_id} limit 1");
            return !empty($result_array)? array_shift($result_array) : false;
        }
        public static function find_by_profile_id($profile_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where profile_id={$profile_id}");
		return $result_array;
	}
        public static function find_by_profile_id_type_id($profile_id=0,$type=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where profile_id={$profile_id} and type={$type}");
		return $result_array;
	}
