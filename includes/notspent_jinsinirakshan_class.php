...
class Notspentjinsinirakshan
{
	protected static $table_name = "notspent_jinsinirakshan";
	protected static $db_fields = array('id','nirikshan_id','prev_stock','spent_item_id','specification_type'
                ,'reduce_amount','increased_amount','total_amount','current_status_active','current_status_inactive'
            ,'remarks','created_date','created_date_english');
	    public $id;
        public $nirikshan_id;
        public $prev_stock;
	    public $spent_item_id;
        public $specification_type;
	    public $reduce_amount;
        public $increased_amount;
        public $total_amount;
        public $current_status_active;
        public $current_status_inactive;
        public $remarks;
        public $created_date;
        public $created_date_english;
        // Common database method
         public static function get_max_nisikshan_id()
        {
            global $database;
            $sql = "select max(nirikshan_id) from ".self::$table_name. " ";
	...
	public function getObjectsEmptyProperties()
	{
		...
		return $obj;
         ...
         public static function find_by_nirikshan_id($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where nirikshan_id={$id} ");
		return $result_array;
	}
         public static function find_by_nirikshan_ids($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where nirikshan_id={$id} ");
		return !empty($result_array)? array_shift($result_array) : false;
