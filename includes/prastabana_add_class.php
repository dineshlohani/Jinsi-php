...
class Prastabanaadd
{
	protected static $table_name = "prastabana_add";
	protected static $db_fields = array('id','kharid_id','organization_id','prastabana_entry_date','prastabana_entry_date_english','prastabana_prakar_id');
	public $id;
	public $kharid_id;
    public $organization_id;
    public $prastabana_entry_date;
    public $prastabana_entry_date_english;
        ...
        public static function find_org($id=0)
	{
		...
		return $result_array;
         ...
         public static function find_by_kharid_ids($kharid_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where kharid_id={$kharid_id} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
        public static function find_by_kharid_id($kharid_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where kharid_id={$kharid_id}");
		return $result_array;
	}
         public static function check_mag_form_id($kharid_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where kharid_id={$kharid_id}");
		return !empty($result_array)? TRUE : FALSE;
	}
        public static function find_by_kharid_id_org_id($kharid_id=0,$org_id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where kharid_id={$kharid_id} and organization_id={$org_id} limit 1");
		return !empty($result_array)? array_shift($result_array) : false;
	}
