...
class Spentmaujadbarsikkharcha
{
	protected static $table_name = "spent_maujad_barsik_kharcha";
	protected static $db_fields = array('id','maujad_id','prev_stock','spent_item_id','in_use','not_in_use','to_repair','not_to_repair','remarks','created_date','created_date_english');
	    public $id;
        public $maujad_id;
        public $prev_stock;
        ...
        public $not_in_use;
        public $to_repair;
        public $not_to_repair;
        public $remarks;
        public $created_date;
        public $created_date_english;
        // Common database method
         public static function get_max_maujad_id()
        {
            global $database;
            $sql = "select max(maujad_id) from ".self::$table_name. " ";
	    ...
	    return !empty($result_array)? array_shift($result_array) : true;
        }
	...
	public function getObjectsEmptyProperties()
	{
        ...
        public static function find_by_maujad_id($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where maujad_id={$id}");
		return $result_array;
	}
        public static function find_by_maujad_ids($id=0)
	{
		global $database;
		$result_array=self::find_by_sql("select * from ".self::$table_name. " where maujad_id={$id}");
