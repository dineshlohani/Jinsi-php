...
if(isset($_GET['jinsi_lilam_id']))
{
    ...
    $approve_result = Jinsililamfinal::find_by_sql("select * from jinsi_lilam_final where jinsi_lilam_id=".$_GET['jinsi_lilam_id']);
    if(!empty($approve_result))
    {
        echo alertBox("निम्न लिलाम फारम स्वीकृत भै सकेको छ ...","jinsi_lilam_final.php");
    }
    $lilam_result = Jinsililam::find_by_sql("select * from jinsi_lilam_approve where jinsi_lilam_id=".$_GET['jinsi_lilam_id']);
}
if(isset($_POST['submit']))
{
    for($i=0;$i<count($_POST['stock_id']);$i++)
    {
        $data=new Jinsililamfinal();
    ...
    echo alertBox("थप सफल ", "jinsi_lilam_final.php");
}
$item_type = Itemtype::find_all();
$datas= Enlist::find_all();
$jinsi_lilam_result = Jinsililamapprove::find_by_sql("select distinct jinsi_lilam_id from jinsi_lilam_approve");
?>
...
<body>
    ...
    <div id="body_wrap_inner"> 
    	<div class="maincontent">
            <h2 class="headinguserprofile">जिन्सी लिलाम गर्नुहोस | <a href="jinsi_lilam_dashboard.php" class="btn">पछि जानुहोस </a> </h2>
                    <div class="OurContentFull">
                    	<h2>लिलाम गर्नुहोस</h2>
                        <div class="userprofiletable">
                          ...
                          <?php if(!isset($_GET['submit']) && !isset($_GET['jinsi_lilam_id'])):?>
                            <table class="table table-responsive bordereless search_table">
                                ...
                                <tr>
                                    ...
                                    <td><a href="jinsi_lilam_final.php?jinsi_lilam_id=<?=$data->jinsi_lilam_id?>" class="btn">खोज्नुहोस</a></td>
                                </tr>
                                <?php endforeach;?>
                            </table>
                            <?php endif;?>
                          ...
                          <table class="table table-bordered table-responsive myWidth100">
                              ...
                              <tr>
                                ...
                                <th class="myCenter">जिन्सी लिलाम भएको परमाण </th>                                
                            </tr>
...
//                                        echo $total_year;exit;
                            ...
                            <tr>
                                ...
                                <td><input type="text" name="stock_entry_date[]" readonly="true" value="<?= $dakhila_date?>"</td>
                               ...
                               <td><input type="text" name="prev_stock[]" value="<?=$data->prev_stock?>" readonly="true" id="prev_stock_<?=$i?>"></td>
                                ...
                                <td><?php echo convertedcit($amount);?></td>
                                    <td><input type="text"  name="reduce_stock[]" readonly="true" id="reduce_lilam_stock_<?=$i?>" value="<?=$data->reduce_stock?>"></td>
                                <td><input type="text" name="current_analysed_rate[]" value="<?=$data->current_analysed_rate?>" readonly="true"></td>
                                <td><input type="text" name="actual_reduced_stock[]" id="actual_reduced_stock_<?=$i?>"/></td>
                          ...
                          <table class="table table-bordered">
                          	<tr>
                                    <td  class="myCenter"><input type="submit" name="submit" value="सेभ गर्नुहोस" class="btn" onclick="confirm('तपाई STOCK घटाउदै हुनुहुन्छ.. के निश्चित हुनुहुन्छ..?');"></a></td>
