...
//    echo "<pre>"; print_r($_POST); echo "</pre>"; exit;
    $profile =  KharidIkaiProfile::find_by_id($_POST['profile_id']);
    $profile->date_nepali = $_POST['date_nepali'];
    $profile->date_english = DateNepToEng($_POST['date_nepali']);
    $profile_id = $profile->save();
    $edit_details = KharidIkaiDetails::find_by_profile_id($_POST['profile_id']);
    ...
    {
        $details = new KharidIkaiDetails;
        $details->worker_id = $_POST['worker_id'][$i];
        $details->pad_id = $_POST['pad_id'][$i];
        $details->profile_id = $_POST['profile_id'];
        $details->save();
    }
    echo alertBox("कार्य सफल", "kharid_ikai.php");
}

$profile = KharidIkaiProfile::find_by_id( (int) $_GET['id']);
